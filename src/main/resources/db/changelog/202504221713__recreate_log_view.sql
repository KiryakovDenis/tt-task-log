...
data_with_changes AS (
    SELECT a1.operation
          ...
          ,lag(a1.title) OVER (PARTITION BY a1.id ORDER BY a1.log_time) AS prev_title
          ,a1.author AS curr_author
          ,lag(a1.author) OVER (PARTITION BY a1.id ORDER BY a1.log_time) AS prev_author
          ,a1.editor AS curr_editor
          ,lag(a1.editor) OVER (PARTITION BY a1.id ORDER BY a1.log_time) AS prev_editor
          ,a1.status AS curr_status
          ,lag(a1.status) OVER (PARTITION BY a1.id ORDER BY a1.log_time) AS prev_status
          ,a1.assignee AS curr_assignee
          ,lag(a1.assignee) OVER (PARTITION BY a1.id ORDER BY a1.log_time) AS prev_assignee
          ,a1.dead_line AS curr_dead_line
          ,lag(a1.dead_line) OVER (PARTITION BY a1.id ORDER BY a1.log_time) AS prev_dead_line
          ,a1.created_at AS curr_created_at
          ,lag(a1.created_at) OVER (PARTITION BY a1.id ORDER BY a1.log_time) AS prev_created_at
          ,a1.updated_at AS curr_updated_at
          ,lag(a1.updated_at) OVER (PARTITION BY a1.id ORDER BY a1.log_time) AS prev_updated_at
          ,a1.description AS curr_description
          ,lag(a1.description) OVER (PARTITION BY a1.id ORDER BY a1.log_time) AS prev_description
      FROM data_ a1),
pure_cahnges AS (
  SELECT a.operation
        ,a.log_time
...
SELECT a2.id, a2.log_time, a2.operation, jsonb_build_object('change_event', jsonb_agg(a2.change_event)) AS event_log
  FROM (SELECT a1.id, a1.log_time, a1.operation,
 ...
 GROUP BY a2.id, a2.log_time, a2.operation